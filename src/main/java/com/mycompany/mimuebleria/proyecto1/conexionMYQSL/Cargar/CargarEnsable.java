 ...
 * @author drymnz
 */
public class CargarEnsable {

    private Connection conexion;
    private PreparedStatement ingresar;

    // constructor 
    public CargarEnsable(Connection coneccion) {
    ...
    public boolean cargarEnsablePiezas(Pieza pieza, Mueble mueble, int cantidad) {
        try {
            ingresar = conexion.prepareStatement("INSERT INTO ensablepieza VALUES (?,?,?)");
    ...
    public boolean cargarEnsableMueble(Mueble mueble, Usuario ensablador, LocalDate fecha) {
        try {
            ingresar = conexion.prepareStatement("INSERT INTO ensableMueble VALUES (?,?,?)");
